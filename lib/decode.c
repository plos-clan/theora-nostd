...
static const unsigned char OC_MODE_ALPHABETS[7][OC_NMODES] = {
    /*Last MV dominates */
    {OC_MODE_INTER_MV_LAST, OC_MODE_INTER_MV_LAST2, OC_MODE_INTER_MV,       OC_MODE_INTER_NOMV,
     OC_MODE_INTRA,          OC_MODE_GOLDEN_NOMV, OC_MODE_GOLDEN_MV, OC_MODE_INTER_MV_FOUR},
    {OC_MODE_INTER_MV_LAST, OC_MODE_INTER_MV_LAST2, OC_MODE_INTER_NOMV,     OC_MODE_INTER_MV,
     OC_MODE_INTRA,          OC_MODE_GOLDEN_NOMV, OC_MODE_GOLDEN_MV, OC_MODE_INTER_MV_FOUR},
    {OC_MODE_INTER_MV_LAST, OC_MODE_INTER_MV,       OC_MODE_INTER_MV_LAST2, OC_MODE_INTER_NOMV,
     OC_MODE_INTRA,          OC_MODE_GOLDEN_NOMV, OC_MODE_GOLDEN_MV, OC_MODE_INTER_MV_FOUR},
    {OC_MODE_INTER_MV_LAST, OC_MODE_INTER_MV,       OC_MODE_INTER_NOMV,     OC_MODE_INTER_MV_LAST2,
     OC_MODE_INTRA,          OC_MODE_GOLDEN_NOMV, OC_MODE_GOLDEN_MV, OC_MODE_INTER_MV_FOUR},
    /*No MV dominates.*/
    {OC_MODE_INTER_NOMV,    OC_MODE_INTER_MV_LAST,  OC_MODE_INTER_MV_LAST2, OC_MODE_INTER_MV,
     OC_MODE_INTRA,          OC_MODE_GOLDEN_NOMV, OC_MODE_GOLDEN_MV, OC_MODE_INTER_MV_FOUR},
    {OC_MODE_INTER_NOMV,    OC_MODE_GOLDEN_NOMV,    OC_MODE_INTER_MV_LAST,  OC_MODE_INTER_MV_LAST2,
     OC_MODE_INTER_MV,       OC_MODE_INTRA,       OC_MODE_GOLDEN_MV, OC_MODE_INTER_MV_FOUR},
    /*Default ordering.*/
    {OC_MODE_INTER_NOMV,    OC_MODE_INTRA,          OC_MODE_INTER_MV,       OC_MODE_INTER_MV_LAST,
     OC_MODE_INTER_MV_LAST2, OC_MODE_GOLDEN_NOMV, OC_MODE_GOLDEN_MV, OC_MODE_INTER_MV_FOUR}
};
...
/*The number of additional extra bits that are decoded with each of the
   internal DCT tokens.*/
static const unsigned char OC_INTERNAL_DCT_TOKEN_EXTRA_BITS[15] = {12, 4, 3, 3, 4, 4, 5, 5,
                                                                   8,  8, 8, 8, 3, 3, 6};

/*Whether or not an internal token needs any additional extra bits.*/
#define OC_DCT_TOKEN_NEEDS_MORE(token)                                                             \
  (token <                                                                                         \
   (int)(sizeof(OC_INTERNAL_DCT_TOKEN_EXTRA_BITS) / sizeof(*OC_INTERNAL_DCT_TOKEN_EXTRA_BITS)))
...
#define OC_DCT_CW_PACK(_eobs, _rlen, _mag, _flip)                                                  \
  ((_eobs) << OC_DCT_CW_EOB_SHIFT | (_rlen) << OC_DCT_CW_RLEN_SHIFT |                              \
   (_flip) << OC_DCT_CW_FLIP_BIT | (_mag) - (_flip) << OC_DCT_CW_MAG_SHIFT)
...
static int oc_sb_run_unpack(oc_pack_buf *_opb) {
  ...
  static const ogg_int16_t OC_SB_RUN_TREE[22] = {4,
                                                 -(1 << 8 | 1),
                                                 -(1 << 8 | 1),
                                                 -(1 << 8 | 1),
                                                 -(1 << 8 | 1),
...
static int oc_block_run_unpack(oc_pack_buf *_opb) {
  /*Coding scheme:
     ...
     11110xx                 11-14
     11111xxxx               15-30*/
  static const ogg_int16_t OC_BLOCK_RUN_TREE[61] = {5,
                                                    -(2 << 8 | 1),
                                                    -(2 << 8 | 1),
                                                    -(2 << 8 | 1),
                                                    -(2 << 8 | 1),
...
static int oc_dec_init(oc_dec_ctx *_dec, const th_info *_info, const th_setup_info *_setup) {
  ...
  /*For each fragment, allocate one byte for every DCT coefficient token, plus
     ...
     one.*/
  _dec->dct_tokens = (unsigned char *)_ogg_malloc((64 + 64 + 1) * _dec->state.nfrags *
...
/*Coding scheme:
   ...
   1111110                 6
   1111111                 7*/
static const ogg_int16_t OC_VLC_MODE_TREE[26] = {4,
                                                 -(1 << 8 | 0),
                                                 -(1 << 8 | 0),
                                                 -(1 << 8 | 0),
                                                 -(1 << 8 | 0),
...
static const ogg_int16_t OC_CLC_MODE_TREE[9] = {3,
                                                -(3 << 8 | 0),
                                                -(3 << 8 | 1),
                                                -(3 << 8 | 2),
                                                -(3 << 8 | 3),
...
static const ogg_int16_t OC_VLC_MV_COMP_TREE[101] = {5,
                                                     -(3 << 8 | 32 + 0),
                                                     -(3 << 8 | 32 + 0),
                                                     -(3 << 8 | 32 + 0),
                                                     -(3 << 8 | 32 + 0),
...
static const ogg_int16_t OC_CLC_MV_COMP_TREE[65] = {6,
                                                    -(6 << 8 | 32 + 0),
                                                    -(6 << 8 | 32 - 0),
                                                    -(6 << 8 | 32 + 1),
                                                    -(6 << 8 | 32 - 1),
...
static int oc_dec_postprocess_init(oc_dec_ctx *_dec) {
  ...
  if (_dec->dc_qis == NULL) {
    ...
    coded_fragis = _dec->state.coded_fragis;
    ncoded_fragis =
        _dec->state.ncoded_fragis[0] + _dec->state.ncoded_fragis[1] + _dec->state.ncoded_fragis[2];
    ...
    if (fragy == 0) {
      ...
      for (fragx = 0; fragx < nhfrags; fragx++, fragi++) {
        ...
        if (frags[fragi].coded) {
          ...
          switch ((l_ref == refi) | (ul_ref == refi) << 1 | (u_ref == refi) << 2 |
                  (ur_ref == refi) << 3) {
          ...
          case 14: {
            pred =
                (3 * (u_frags[fragi - 1].dc + u_frags[fragi + 1].dc) + 10 * u_frags[fragi].dc) / 16;
          } break;
          case 7:
          case 15: {
            int p0;
...
static void oc_dec_frags_recon_mcu_plane(oc_dec_ctx *_dec, oc_dec_pipeline_state *_pipe, int _pli) {
  ...
  if (_pipe->nuncoded_fragis[_pli] > 0) {
    ...
    oc_frag_copy_list(&_dec->state, _dec->state.ref_frame_data[OC_FRAME_SELF],
                      ...
                      _pipe->uncoded_fragis[_pli], _pipe->nuncoded_fragis[_pli],
                      _dec->state.frag_buf_offs);
  }
}

/*Filter a horizontal block edge.*/
static void oc_filter_hedge(unsigned char *_dst, int _dst_ystride, const unsigned char *_src,
                            int _src_ystride, int _qstep, int _flimit, int *_variance0,
                            int *_variance1) {
  unsigned char       *rdst;
    ...
    *_variance1 += OC_MINI(255, sum1);
    if (sum0 < _flimit && sum1 < _flimit && r[5] - r[4] < _qstep && r[4] - r[5] < _qstep) {
      ...
      *cdst  = (unsigned char)(r[0] * 2 + r[1] + r[2] * 2 + r[3] + r[4] + r[5] + 4 >> 3);
      cdst  += _dst_ystride;
      for (by = 0; by < 4; by++) {
        *cdst  = (unsigned char)(r[by] + r[by + 1] + r[by + 2] + r[by + 3] * 2 + r[by + 4] +
                                    r[by + 5] + r[by + 6] + 4 >>
                                3);
        cdst  += _dst_ystride;
      }
...
static void oc_filter_vedge(unsigned char *_dst, int _dst_ystride, int _qstep, int _flimit,
                            int *_variances) {
  unsigned char       *rdst;
  const unsigned char *rsrc;
  unsigned char       *cdst;
  ...
  for (by = 0; by < 8; by++) {
    ...
    if (sum0 < _flimit && sum1 < _flimit && r[5] - r[4] < _qstep && r[4] - r[5] < _qstep) {
      ...
      *rdst++ = (unsigned char)(r[0] * 2 + r[1] + r[2] * 2 + r[3] + r[4] + r[5] + 4 >> 3);
      for (bx = 0; bx < 4; bx++) {
        *rdst++ = (unsigned char)(r[bx] + r[bx + 1] + r[bx + 2] + r[bx + 3] * 2 + r[bx + 4] +
                                      r[bx + 5] + r[bx + 6] + 4 >>
                                  3);
      }
...
static void oc_dec_deblock_frag_rows(oc_dec_ctx *_dec, th_img_plane *_dst, th_img_plane *_src,
  ...
  for (; y < y_end; y += 8) {
    ...
    flimit = (qstep * 3) >> 2;
    oc_filter_hedge(dst, dst_ystride, src - src_ystride, src_ystride, qstep, flimit, variance,
                    variance + nhfrags);
    variance++;
    dc_qi++;
    for (x = 8; x < width; x += 8) {
      qstep  = _dec->pp_dc_scale[*dc_qi];
      flimit = (qstep * 3) >> 2;
      oc_filter_hedge(dst + x, dst_ystride, src + x - src_ystride, src_ystride, qstep, flimit,
                      variance, variance + nhfrags);
...
static void oc_dering_block(unsigned char *_idata, int _ystride, int _b, int _dc_scale,
                            int _sharp_mod, int _strong) {
...
static void oc_dec_dering_frag_rows(oc_dec_ctx *_dec, th_img_plane *_img, int _pli, int _fragy0,
                                    int _fragy_end) {
  th_img_plane      *iplane;
  oc_fragment_plane *fplane;
  oc_fragment       *frag;
...
int th_decode_ctl(th_dec_ctx *_dec, int _req, void *_buf, size_t _buf_sz) {
  ...
  case TH_DECCTL_SET_GRANPOS: {
    ...
    _dec->state.keyframe_num =
        (granpos >> _dec->state.info.keyframe_granule_shift) - _dec->state.granpos_bias;
...
static void oc_render_telemetry(th_dec_ctx *_dec, th_ycbcr_buffer _ycbcr, int _telemetry) {
  ...
  for (y = 0; y < h; y++) {
    for (x = 0; x < w; x++) {
      ...
      g = (3827562 * y_row[x] - 1287801 * u_row[x >> hdec] - 2672387 * v_row[x >> hdec] +
           447306710) /
          3287200;
  ...
  case TH_PF_420: {
    for (y = 0; y < h; y += 2) {
      ...
      for (x = 0; x < w; x += 2) {
        ...
        u             = (-8372 * (rgb_row[4 * x + 2] + rgb_row[4 * x + 6] + rgb_row2[4 * x + 2] +
                      rgb_row2[4 * x + 6]) -
             16436 * (rgb_row[4 * x + 1] + rgb_row[4 * x + 5] + rgb_row2[4 * x + 1] +
                      rgb_row2[4 * x + 5]) +
             24808 * (rgb_row[4 * x + 0] + rgb_row[4 * x + 4] + rgb_row2[4 * x + 0] +
                      rgb_row2[4 * x + 4]) +
             29032005) /
            225930;
        v = (39256 * (rgb_row[4 * x + 2] + rgb_row[4 * x + 6] + rgb_row2[4 * x + 2] +
                      rgb_row2[4 * x + 6]) -
             32872 * (rgb_row[4 * x + 1] + rgb_row[4 * x + 5] + rgb_row2[4 * x + 1] +
                      rgb_row2[4 * x + 5]) -
             6384 * (rgb_row[4 * x + 0] + rgb_row[4 * x + 4] + rgb_row2[4 * x + 0] +
                     rgb_row2[4 * x + 4]) +
             45940035) /
            357510;
...
int th_decode_packetin(th_dec_ctx *_dec, const ogg_packet *_op, ogg_int64_t *_granpos) {
  ...
  if (_dec->state.frame_type != OC_INTRA_FRAME && (_dec->state.ref_frame_idx[OC_FRAME_GOLD] < 0 ||
                                                   _dec->state.ref_frame_idx[OC_FRAME_PREV] < 0)) {
  ...
  if (_dec->state.ntotal_coded_fragis <= 0) {
    ...
    /*Save the current telemetry state.
      This prevents it from being modified in the middle of decoding this
       ...
       callback.*/
    telemetry =
        _dec->telemetry_mbmode || _dec->telemetry_mv || _dec->telemetry_qi || _dec->telemetry_bits;
#endif
    ...
    for (stripe_fragy = 0; notdone; stripe_fragy += _dec->pipe.mcu_nvfrags) {
      ...
      for (pli = 0; pli < 3; pli++) {
        ...
        _dec->pipe.fragy_end[pli] = OC_MINI(
            fplane->nvfrags, _dec->pipe.fragy0[pli] + (_dec->pipe.mcu_nvfrags >> frag_shift));
        ...
        if (_dec->pipe.loop_filter) {
          sdelay += notstart;
          edelay += notdone;
          oc_state_loop_filter_frag_rows(&_dec->state, _dec->pipe.bounding_values, OC_FRAME_SELF,
                                         pli, _dec->pipe.fragy0[pli] - sdelay,
        ...
        avail_fragy_end =
            OC_MINI(avail_fragy_end, _dec->pipe.fragy_end[pli] - edelay << frag_shift);
      }
#ifdef HAVE_CAIRO
    ...
    if (telemetry) {
      ...
      if (_dec->stripe_cb.stripe_decoded != NULL) {
        (*_dec->stripe_cb.stripe_decoded)(_dec->stripe_cb.ctx, stripe_buf, 0,
                                          _dec->state.fplanes[0].nvfrags);
      }
    }
#endif
#if defined(OC_DUMP_IMAGES)
